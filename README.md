...
AIInference aims to be a high-performance machine learning library written in C++. The long-term goal is to provide efficient implementations of core ML algorithms and primitives from scratch.

Currently, the library features optimised matrix multiplication that performs **up to 50x faster** than a naive implementation. This is achieved through two key techniques:

- **SIMD (Single Instruction, Multiple Data)**: Leverages ARM NEON vector instructions (`armv8.4-a+simd`) to process multiple floating-point values in parallel within a single CPU instruction.
