#include "tensor.h"
#include "strategies/basic_tensor_strategy.h"
#include "strategies/simd_tensor_strategy.h"
#include "strategies/concurrent_row_tensor_strategy.h"
#include "strategies/optimised_tensor_strategy.h"
#include "strategies/blocked_simd_tensor_strategy.h"

#include <gtest/gtest.h>

class MatrixMultTest : public testing::TestWithParam<TensorStrategy*> {};

TEST_P(MatrixMultTest, SmallSquareMatrixMultTest) {
    Tensor a = Tensor({1.0f, 2.0f, 3.0f, 4.0f}, 2, 2, *GetParam());
    Tensor b = Tensor({5.0f, 6.0f, 7.0f, 8.0f}, 2, 2, *GetParam());

    Tensor result = a * b;
    ASSERT_EQ(result.getWidth(), 2);
    ASSERT_EQ(result.getHeight(), 2);

    float expected[4] = {19.0f, 22.0f, 43.0f, 50.0f};

    int j=0;
    for (auto r : result) {
        ASSERT_FLOAT_EQ(r, expected[j]);
        j++;
    }

}

TEST_P(MatrixMultTest, SmallRectMatrixMultTest) {
    Tensor a = Tensor({1.0f, 2.0f, 3.0f, 4.0f}, 2, 2, *GetParam());
    Tensor b = Tensor({5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f}, 2, 4, *GetParam());

    Tensor result = a * b;
    ASSERT_EQ(result.getWidth(), 4);
    ASSERT_EQ(result.getHeight(), 2);

    float expected[8] = {23.0f, 26.0f, 29.0f, 32.0f, 51.0f, 58.0f, 65.0f, 72.0f};

    int j=0;
    for (auto r : result) {
        ASSERT_FLOAT_EQ(r, expected[j]);
        j++;
    }

}

TEST_P(MatrixMultTest, LargeSquareMatrixMultTest) {

    int n = 500;

    std::vector<float> a = std::vector<float>(n*n, 0.0f);
    for (unsigned long i=0; i<n*n; i+=n+1) {
        a[i] = 1.0f;
    }

    std::vector<float> b = std::vector<float>(n*n, 0.0f);
    for (unsigned long i=0; i<n*n; i++) {
        b[i] = static_cast<float>(i);
    }

    Tensor A = Tensor(a, n, n, *GetParam());
    Tensor B = Tensor(b, n, n, *GetParam());

    Tensor result = A * B;
    ASSERT_EQ(result.getWidth(), n);
    ASSERT_EQ(result.getHeight(), n);

    int j=0;
    for (auto r : result) {
        ASSERT_FLOAT_EQ(r, static_cast<float>(j));
        j++;
    }
}

TEST_P(MatrixMultTest, LargeRectMatrixMultTest) {
    
    int n = 250000;

    std::vector<float> a = std::vector<float>(250000, 1.0f);
    std::vector<float> b = std::vector<float>(250000, 1.0f);

    Tensor A = Tensor(a, 1, 250000, *GetParam());
    Tensor B = Tensor(b, 250000, 1, *GetParam());

    Tensor result = A * B;
    ASSERT_EQ(result.getWidth(), 1);
    ASSERT_EQ(result.getHeight(), 1);

    ASSERT_FLOAT_EQ(*result.begin(), 250000.0f);
    
}


// Test whether loop blocking and SIMD vectorization works with matrices
// whose numbers of elements are not divisible by the block size or 
// number of SIMD registers
TEST_P(MatrixMultTest, PrimeSizeMatrixMultTest) {
    std::vector<float> a = std::vector<float>(67, 1.0f);
    std::vector<float> b = std::vector<float>(67, 1.0f);

    Tensor A = Tensor(a, 67, 1, *GetParam());
    Tensor B = Tensor(b, 1, 67, *GetParam());

    Tensor result = A * B;
    ASSERT_EQ(result.getWidth(), 67);
    ASSERT_EQ(result.getHeight(), 67);

    for (auto r : result) {
        ASSERT_FLOAT_EQ(r, 1.0f);
    }
}

TEST_P(MatrixMultTest, NegativeValuesMatrixMultTest) {
    
    int n = 500;

    std::vector<float> a = std::vector<float>(n*n, -3.0f);
    std::vector<float> b = std::vector<float>(n*n, -67.0f);

    Tensor A = Tensor(a, n, n, *GetParam());
    Tensor B = Tensor(b, n, n, *GetParam());

    Tensor result = A * B;
    ASSERT_EQ(result.getWidth(), n);
    ASSERT_EQ(result.getHeight(), n);

    for (auto r : result) {
        ASSERT_FLOAT_EQ(r, n * -3.0f * -67.0f);
    }
}

TEST_P(MatrixMultTest, LargeValuesMatrixMultTest) {

    int n = 10;

    std::vector<float> a = std::vector<float>(n*n, 20000.0f);
    std::vector<float> b = std::vector<float>(n*n, 10000.0f);

    Tensor A = Tensor(a, n, n, *GetParam());
    Tensor B = Tensor(b, n, n, *GetParam());

    Tensor result = A * B;
    ASSERT_EQ(result.getWidth(), n);
    ASSERT_EQ(result.getHeight(), n);

    for (auto r : result) {
        ASSERT_FLOAT_EQ(r, 20000.0f * 10000.0f * n);
    }
}

TEST_P(MatrixMultTest, InvalidMultTest) {
    std::vector<float> a = std::vector<float>(10000, 1.0f);
    std::vector<float> b = std::vector<float>(10000, 1.0f);

    Tensor A = Tensor(a, 1, 10000, *GetParam());
    Tensor B = Tensor(b, 1, 10000, *GetParam());

    ASSERT_THROW(A * B, std::runtime_error);
}

INSTANTIATE_TEST_CASE_P(TestAllMultStrategies, MatrixMultTest, testing::Values(
    &BasicTensorStrategy::get_instance(),
    &SimdTensorStrategy::get_instance(),
    &ConcurrentRowTensorStrategy::get_instance(),
    &OptimisedTensorStrategy::get_instance(),
    &BlockedSimdTensorStrategy::get_instance()
));


